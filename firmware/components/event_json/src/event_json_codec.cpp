...
std::optional<std::string> EventJsonCodec::serialize(const event::Event& event) {
    ...
    if (cJSON_AddStringToObject(json.get(), "event_id", event.event_id().data()) ==
            nullptr ||
        cJSON_AddStringToObject(
            json.get(), "event_type", event.event_type().data()) == nullptr ||
        cJSON_AddNumberToObject(
            json.get(), "occurred_at_unix_ms", static_cast<double>(timestamp)) ==
            nullptr ||
        cJSON_AddStringToObject(
            json.get(),
            "originating_device",
    ...
    std::unique_ptr<char, decltype(&cJSON_free)> serialized(
        cJSON_PrintUnformatted(json.get()),
        cJSON_free);
    if (!serialized) {
        return std::nullopt;
    }
